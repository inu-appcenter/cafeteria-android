...
public class CoverFlowView extends RelativeLayout {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////
    // Custom setting for INU-Cafeteria Application ////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void setCafeList(String[] coverflowTitles) {
        this.coverflowTitles = coverflowTitles;
    }
    public void setCafeCode(String[] cafeCode) { this.cafeCode =  cafeCode; }

    private ListView listView;
    private JsonObject jsonObject;
    private int[] coverflowIsMenu;
    private FoodMenuListViewAdpater adpater;
    public void setFoodMenuView(ListView listView) {
        this.listView = listView;
    }
    public void setFoodMenu(JsonObject jsonObject) { this.jsonObject = jsonObject; }
    public void setCoverflowIsMenu(int[] coverflowIsMenu) { this.coverflowIsMenu = coverflowIsMenu; }
    public void setFoodMenuAdapter(FoodMenuListViewAdpater adapter) {
    ...
    public void setCoverflowIsAlarm(boolean[] coverflowIsAlarm) { this.coverflowIsAlarm = coverflowIsAlarm; }
    public void setSubmitButton(Button submitButton) {
        this.submitButton = submitButton;
    }

    private String curCafeName;
    ...
    public void computeScroll() {
        ...
        // 반복되는 view 갱신을 피하기 위함
        if(cafeCode == null || curCafeName == cafeCode[getTopViewPosition()]) {
            return ;
        } else {
            curCafeName = cafeCode[getTopViewPosition()];
        }

        // coverflow 아래 식당 이름 바꾸기
        if(textView != null && coverflowTitles != null) {
            textView.setText(coverflowTitles[getTopViewPosition()]);
        }

        // 식당 메뉴 바꾸기
        if(listView !=  null && jsonObject != null) {
            if(coverflowIsMenu[getTopViewPosition()] != -1) {
                listView.setVisibility(View.VISIBLE);
                JsonArray oneCafeteriaMenuInfo = jsonObject.get(cafeCode[getTopViewPosition()]).getAsJsonArray();
                adpater.setItems(oneCafeteriaMenuInfo);
                adpater.notifyDataSetChanged();
            } else  {
                // 식당 메뉴가 없을 경우 listview가 숨겨지며 뒤에 숨겨져있던 '서비스 준비중입니다' 라고 써진 TextView가 보인다
                listView.setVisibility(View.INVISIBLE);
            }
        }

        if(submitButton == null || coverflowIsAlarm == null) return ;

        // if no alarm, lock the submit button
        if(coverflowIsAlarm[getTopViewPosition()]) {
            // The menu is exists
            submitButton.setClickable(true);
            submitButton.setTextColor(getResources().getColor(R.color.pointBlue));
        } else {
            // The menu does not exist
            submitButton.setClickable(false);
            submitButton.setTextColor(getResources().getColor(R.color.lightGray));
        }
