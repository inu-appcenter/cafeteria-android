...
아주 소소한 커밋들은 master에 직접 올립니다. 예를 들어 README.md를 수정하는 등의 작업은

master 브랜치에서 직접 작업합니다.

master 브랜치가 출시 준비가 되었다고 판단되면 새로운 tag과 release를 생성합니다.
...
보통의 모바일 애플리케이션은 하단 또는 상단의 탭으로 네비게이션을 제어합니다.

이곳에 담기 힘든 컨텐츠는 별도의 페이지에 세로 스크롤 형식으로 배치합니다.

모바일 웹 초기에 도입되어 지금도 널리 쓰이는 Drawer, 일명 Hamburger Menu 또한 하나의 방법입니다.

여러 개의 메뉴를 필요할 때에만 보여줄 수 있다는 장점이 있습니다.

하지만 그 점이 단점으로 작용하기도 합니다.

사용자가 Drawer를 여는 버튼을 누르지 않는 이상 그 속에 숨겨져 있는 기능은 사용자에게 인지되지 못합니다.

바코드는 이 앱의 중요한 기능입니다. 주요한 기능은 Drawer 밖으로 나오는 것이 좋다고 판단했습니다.

바코드는 탭과 같이 영속적으로 유지되어야 할 필요가 없습니다. 다만 쉽게 띄울 수 있어야 하며

빠르게 띄울 수 있어야 합니다.

따라서 앱 실행 후 등장하는 메인화면 하단에 Floating Action Button을 배치하여

바코드를 바로 띄울 수 있도록 하였습니다.
